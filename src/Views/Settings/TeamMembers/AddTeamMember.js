          ...
          for (var i = 0; i < errorKeys.length; i++) {
            switch (errorKeys[i]) {
              ...
              case "email":
                ...
                break;
              case "role":
                setRoleErrorMessage(errorMessages[i]);
                break;
              default:
                history.push(GetErrorHandlingRoute(e));
                break;
  ...
  return (
    <>
      ...
      <Container style={useStyles.marginrowtop}>
        {isSuccess ? (
          <Success
            okevent={() => history.push("/datahub/settings/4")}
            route={"datahub/settings"}
            imagename={"success"}
            btntext={"ok"}
            heading={"Team Member added successfully !"}
            imageText={"Success!"}
            msg={"You added a team member."}
          ></Success>
        ) : (
          <>
            <AddMemberForm
              ...
              <Col xs={12} sm={12} md={6} lg={6}>
                {firstname && useremail && !isuseremailerror && userrole ? (
                  ...
                  <Button
                    ...
                    style={{ textTransform: "none" }}
                    disabled
                    className="disbalesubmitbtnteam"
                  >
                    {screenlabels.common.submit}
                  </Button>
                )}
            ...
            <Row style={useStyles.marginrowtop8px}>
              ...
              <Col xs={12} sm={12} md={6} lg={6}>
                <Button
                  onClick={() => history.push("/datahub/settings/4")}
                  style={{ textTransform: "none" }}
                  variant="outlined"
                  className="cancelbtnteam"
                >
