...
const MysqlFormForConnection = ({
  ...
  const [errorDatabaseUserNameMysql, seteErrorDatabseUserNameMysql] =
    useState("");
      ...
      .catch((err) => {
        ...
        if (errorKeys.length > 0) {
          for (var i = 0; i < errorKeys.length; i++) {
            ...
            switch (errorKeys[i]) {
              case "database":
                seteErrorDatabaseNameMysql(errorMessages[i]);
                break;
              case "username":
                seteErrorDatabseUserNameMysql(errorMessages[i]);
                break;
              case "password":
                seteErrorUserNameMysql(errorMessages[i]);
                break;
              case "host":
                seteErrorHostMysql(errorMessages[i]);
                break;
              case "port":
                seteErrorPortMysql(errorMessages[i]);
                break;
              //if error occurs Alert will be shown as Snackbar
              default:
            ...
            switch (errorKeys[i]) {
              ...
              default:
                if (err?.response?.status == 401) {
                  ...
                  setMessageForSnackBar(
                    `Some error occurred during exporting with status code ${
                      err?.response?.status ? err?.response?.status : "NA"
                    } :. Please try again!`
                  ); //error message set
                  setErrorOrSuccess("error"); //error type set
    ...
    HTTPService(
      ...
      .then((response) => {
        ...
        if (response.status === 204) {
          console.log("file deleted");
          var filteredArray = mysqlFileList.filter(
          ...
          <Row className="textfield_row">
            <Col lg={6} sm={12}>
              <TextField
                ...
                helperText={
                  errorDatabaseNameMysql ? errorDatabaseNameMysql : ""
                }
                InputProps={{
                  startAdornment: (
              ...
              <TextField
                ...
                helperText={
                  errorDatabaseUserNameMysql ? errorDatabaseUserNameMysql : ""
                }
                InputProps={{
                  startAdornment: (
                    <InputAdornment position="start">
