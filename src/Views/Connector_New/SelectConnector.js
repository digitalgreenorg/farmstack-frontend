...
const SelectConnector = ({ text, organisations, organisationName, setOrganisationName, template, dataset, setDataset, datasets,
    ...
    setIsConditionForConnectorDataForSaveMet, connectorData, setConnectorData, counterForIntegrator
}) => {
    ...
    return (
        <Box>
            ...
            <Affix
                ...
                offsetTop={0}
            >
                <Box className={`${styles.selectors} all_selectors_as_sticky d-flex justify-content-between align-items-baseline mt-20`}>
                    ...
                    <ContainedButton
                        ...
                        width={"171px"}
                        height={"48px"}
                        disabled={connectorData.name &&
                            connectorData.desc && template?.availabeColumns?.length > 0 ? false : true}
