...
export default function DeleteSubCat(props) {
  const {
    ...
    setListSubCategory,
    ListOfSubCategory,
    listOfSubCategoriesForSelected,
  } = props;
        ...
        <DialogContent>
              ...
              >
                {data.map((each, index) => {
                  return (
                    <MenuItem
                      key={index}
                      value={each.category ? each.category : each.sub_category}
                    >
                      {each.category ? each.category : each.sub_category}
                    </MenuItem>
                  );
                })}
              </Select>
          ...
          <Row>
            <FormControl sx={{ m: 1, minWidth: 120 }}>
              ...
              <FormHelperText>
                {helperText ? "Please select anyone sub category" : ""}
              </FormHelperText>
            </FormControl>
            {/* <SelectorForCategories handler={handler} inputLabel={inputLabel} value={value} data={data} helperText={"Please select anyone category"} /> */}
          </Row>
