...
const DatasetIntegration = (props) => {
    ...
    const getFilesAssociatedForTheSelectedDatasets = async (source, list, org, i) => {
        ...
        return await HTTPService(method, url, payload, false, true, false).then((res) => {
            setLoader(false)
            if (source == "dataset") {
                setTemplate({ ...template, availabeColumns: [], dataset_name: res.data[0]?.dataset_name ? res.data[0].dataset_name : "N/A", dataset_id: org ? org : "", file_name: "", availabeColumns: [], file_list: [...res.data] })
            }
            else if (source == "file") {
                let name = list[0]
                let resArr = []
                ...
                setTemplate({ ...template, availabeColumns: [], dataset_name: "", dataset_id: "", file_name: "", file_list: [], dataset_list: [...res.data], org_id: org, org_name: res?.data?.length > 0 ? res.data[0]?.org_name : "" })
            }
            return true
        }).catch((err) => {
            goToTop(0)
    ...
    const completeDataGenerator = (maps) => {
        ...
        for (let i = 0; i < maps.length; i++) {
            ...
            //Current obj
            currentObj["file_name"] = maps[i]?.left_dataset_file?.file ? maps[i]?.left_dataset_file?.file : "N/A"
            currentObj["file_id"] = maps[i]?.left_dataset_file?.id ? maps[i]?.left_dataset_file?.id : ""
            // currentObj["dataset_id"] = maps[i]?.left_dataset_file?.dataset?. ?  maps[i]?.left_dataset_file?.file.split("/").at(-1) : "N/A"
            currentObj["dataset_name"] = maps[i]?.left_dataset_file?.dataset?.name ? maps[i]?.left_dataset_file?.dataset?.name : ""
            currentObj["org_name"] = maps[i]?.left_dataset_file?.dataset?.user_map?.organization?.name ? maps[i]?.left_dataset_file?.dataset?.user_map?.organization?.name : ""
            currentObj["org_id"] = maps[i]?.left_dataset_file?.dataset?.user_map?.id ? maps[i]?.left_dataset_file?.dataset?.user_map?.id : ""
            currentObj["type"] = maps[i]?.condition?.how ? maps[i]?.condition?.how : "left"
            ...
            //Next obj
            nextObj["left"] = maps[i]?.condition?.left?.length > 0 ? maps[i]?.condition?.left : []
            nextObj["file_name"] = maps[i]?.right_dataset_file?.file ? maps[i]?.right_dataset_file?.file : "N/A"
            nextObj["file_id"] = maps[i]?.right_dataset_file?.id ? maps[i]?.right_dataset_file?.id : ""
            nextObj["dataset_name"] = maps[i]?.right_dataset_file?.dataset?.name ? maps[i]?.right_dataset_file?.dataset?.name : ""
            nextObj["columnsSelected"] = maps[i]?.condition?.right_selected.length > 0 ? maps[i]?.condition?.right_selected : []
            nextObj["availabeColumns"] = maps[i]?.condition?.right_available_columns?.length > 0 ? maps[i]?.condition?.right_available_columns : maps[i]?.condition?.left_selected
            nextObj["org_name"] = maps[i]?.right_dataset_file?.dataset?.user_map?.organization?.name ? maps[i]?.right_dataset_file?.dataset?.user_map?.organization?.name : ""
            nextObj["org_id"] = maps[i]?.right_dataset_file?.dataset?.user_map?.id ? maps[i]?.right_dataset_file?.dataset?.user_map?.id : ""
            arr[i + 1] = nextObj
        }
        // let obj = {}
    ...
    return (
        <>
            ...
            {isDatasetIntegrationListModeOn && <span><ConnectorsList setConnectorTimeData={setConnectorTimeData} isEditModeOn={isEditModeOn} setIsEditModeOn={setIsEditModeOn} setConnectorIdForView={setConnectorIdForView} setIsDatasetIntegrationListModeOn={setIsDatasetIntegrationListModeOn} /></span>}
        </>
    )
}
